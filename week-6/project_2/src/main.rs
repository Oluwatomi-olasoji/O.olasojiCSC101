...
fn main() {
   ...
   let mut vecname: Vec<String> = Vec::new();
   let mut agevec: Vec<i32>= Vec::new();
   /*let statusvec: Vec<i64> = Vec::new();
   let offspringvec: Vec<i64> = Vec::new(); */
...
//for loop to ask the questions as many times as the number of siblings
    for num in 1.. numsib1 + 1 {
        ...
        let name1:String = name.trim().parse().expect("Cannot read input");//making a varible to move into the vector
        vecname.push(name1);
